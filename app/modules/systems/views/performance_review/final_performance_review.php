    ...
    <div class="col-md-7 align-self-center text-right">
        <div class="d-flex justify-content-end align-items-center">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Final Performance Reviews</li>
            </ol>
        </div>
    </div>
</div>
    ...
    <div class="col-12">
        <div class="element-wrapper">
            ...
            <div class="card-header bg-info page-head-title-wrap">
                <h4 class="page-head-title card-title  text-white" style="display: inline-block"> Performance Reviews Summary</h4>
            ...
            <div class="modal-body form">
                <div class="form-body" style="padding: 0px 10px;">
                    <div class="row">
                        ...
                        <div class="col-md-5">
                            <div class="form-group">
                                <h5><u>Employee Self Assessment</u></h5>
                        ...
                        <div class="col-md-5">
                            <div class="form-group">
                                <h5><u>Coordinator Review</u></h5>

                                <div id="load_view_quiz_form_data_co"></div>
                                <span class="error-block"></span>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function() {
    ...
    review_datatable = $('#review_datatable').DataTable({
        ...
        "ajax": {
            ...
            "url": "<?=base_url()?>systems/performance_review/get_review_data_summary",
            "type": "POST"
        },
...
function view_submitted_form(id)
{
    ...
    //Ajax Load data from ajax
    $.ajax({
        url : "<?php echo site_url('systems/performance_review/get_review_quiz_data_by_id_final/')?>/" + id,
        type: "GET",
        "data": {
            "<?php echo $this->security->get_csrf_token_name(); ?>": "<?php echo $this->security->get_csrf_hash(); ?>"
        },
        ...
        success: function(data)
        {
            ...
            for(var i=0;i<data.QuizTemplateData.length;i++) {
                ...
                for(var j=0;j<data.QuizTemplateData[i].length;j++) {

                    if (data.QuizTemplateData[i][j].type == 'employee') {
                        // if (data.QuizTemplateData[i].code) {
                        $('#load_view_quiz_form_data').append('' +
                        '<div class="form-group">' +
                        '<label class="control-label col-md-12" for="question' + (j + 1) + '">' + data.QuizTemplateData[i][j].question + '</label>' +
                        '<div class="col-md-12">' +
                        '' + data.QuizTemplateData[i][j].answer + '' +
                        '<span class="help-block"></span>' +
                        '</div>' +
                        '</div>' +
                        '');
                    ...
                    else if (data.QuizTemplateData[i][j].type=='coordinator') {

                        $('#load_view_quiz_form_data_co').append('' +
                        '<div class="form-group">' +
                        '<label class="control-label col-md-12" for="question' + (j + 1) + '">' + data.QuizTemplateData[i][j].question + '</label>' +
                        '<div class="col-md-12">' +
                        '' + data.QuizTemplateData[i][j].answer + '' +
